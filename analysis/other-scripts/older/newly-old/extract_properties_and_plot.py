...
from functions import convert_to_slabnorm_normstress
...
saved_stresses_name = ''.join(['text_files/new2/',model_name,'.z',str(analysis_depth/1.e3),'.shear-dz',str(analysis_depth_dz/1.e3),'.ds',str(ds/1.e3),'.prof-dz',str(dz/1.e3),'km.txt'])
...
saved_stresses = np.zeros(((max_time-first_time),21)) 
                                                      # time, DP_mod_shall, DP_mod_deep, DP_mod, DP_anal, dip_midmant, slab_stress_term, 
                                                      # slab_stress_term_b, slab_stress_term_c, slabnorm_thick, Snorm_contrib, K_midmant, dK_midmant, Pleft, Pright, vc
ind = 0 
...
for time in range(first_time,max_time,1):
    ...
    slab_norm_devstress_shall_cut   = slab_norm_devstress_shall[inds_shall]
    ...
    slab_norm_devstress_deep_cut= slab_norm_devstress_deep[inds_deep]
    ...
    slabnorm_stress_tmp             = trapz(slab_norm_devstress_cut[:,0],       profile_cut[:,2]*1.e3) / (prof_length*1.e3)
    slabnorm_stress_shall_tmp       = trapz(slab_norm_devstress_shall_cut[:,0], profile_shall_cut[:,2]*1.e3) / (prof_length_shall*1.e3)
    slabnorm_stress_deep_tmp        = trapz(slab_norm_devstress_deep_cut[:,0],  profile_deep_cut[:,2]*1.e3) / (prof_length_deep*1.e3)
    slabnorm_stress                 = slabnorm_stress_tmp * slabnorm_thick
    slabnorm_stress_shall           = slabnorm_stress_shall_tmp * slabnorm_thick_shall
    slabnorm_stress_deep            = slabnorm_stress_deep_tmp * slabnorm_thick_deep
    slabnorm_stress_fullterm        = slabnorm_stress * K_midmant
    slabnorm_stress_shall_fullterm  = slabnorm_stress_shall * K_midmant_shall
    slabnorm_stress_deep_fullterm   = slabnorm_stress_deep  * K_midmant_deep

    print("slab norm stress = %.2f MPa m (others: %.2f, %.2f)" % (slabnorm_stress/1.e6,slabnorm_stress_shall/1.e6,slabnorm_stress_deep/1.e6))
    print("slab norm stress term = %.2f MPa (others: %.2f, %.2f)" % (slabnorm_stress_fullterm/1.e6,slabnorm_stress_shall_fullterm/1.e6,slabnorm_stress_deep_fullterm/1.e6))
    ...
    saved_stresses[ind,:] = time, DP_mod_shall, DP_mod_deep, DP_mod, DP_anal, dip_midmant, slab_stress_term, slab_stress_term_b,  \
                            ...
                            Pleft, Pright, slabnorm_stress_fullterm, slabnorm_stress_shall_fullterm, slabnorm_stress_deep_fullterm, vc
    ind = ind + 1
    ...
    ax5.annotate(''.join(['slab shear stress = ',str("%.1f" % (slab_stress_term/1.e6)),', slab norm stress = ',str("%.1f" % (slabnorm_stress_fullterm/1.e6)), 'MPa, misfit = ',str("%.1f" % (misfit/1.e6)),' MPa (K = ',str("%.5f" % (K_midmant*1000)),' 1/km)']), xy=(0.01,0.12), xycoords='axes fraction',verticalalignment='center',horizontalalignment='left',fontsize=5,color='k')        
    ...
    ax1.set_ylim([(ymax-ymin_plot)/1.e3,0])   
    ax1.tick_params(direction='out',length=2, labelsize=6)
    ax1.annotate(''.join([str("%.1f" % (time))]), xy=(0.01,0.6), xycoords='axes fraction',verticalalignment='center',horizontalalignment='left',fontsize=10,color='k')       
    ax1.annotate(''.join(['DP: mod = ',str("%.1f" % (DP_mod/1.e6)),', anal = ',str("%.1f" % (DP_anal/1.e6)),' MPa']), xy=(0.01,0.12), xycoords='axes fraction',verticalalignment='center',horizontalalignment='left',fontsize=8,color='k')        
    ax1.plot(llith_points[:,0], llith_points[:,1], linewidth=0.5, color='black',zorder=6)
    ax1.plot(profile_shall[:,0],profile_shall[:,1], linewidth=0.5, color='green',zorder=6)
    ax1.plot(profile_deep[:,0], profile_deep[:,1],  linewidth=0.5, color='blue',zorder=6)
    ax1.plot(profile[:,0],      profile[:,1],       linewidth=0.5, color='red',zorder=6)
