...
from functions import get_stress_gradient, convert_to_slabnorm_normstress
...
from functions import get_stress_at_certain_depth
...
saved_stresses_name = ''.join(['text_files/new/',model_name,'.z',str(analysis_depth/1.e3),'.shear-dz',str(analysis_depth_dz/1.e3),'.ds',str(ds/1.e3),'.prof-dz',str(dz/1.e3),'km.txt'])
...
first_time=8
saved_stresses = np.zeros(((max_time-first_time),21)) # time [Myr], DP_mod_shall, DP_mod_deep, DP_mod, DP_anal, dip, stress term, stress term B, 
                                                      # stress term C,slabnorm thick, snorm contrib, dgradient normal stress, K, dK
                                                      # vc, slab_viscosity, bending stress, bending/buoyant stress, Psp_mod, Pop_mod, curvature term
ind = 0 
...
for time in range(first_time,max_time,1):
    ...
    # # get slab viscosity
    # slab_viscosity, slab_viscosity_x, slab_viscosity_y = get_slabvisc_from_horiz_prof(midmant_prof,x_col,y_col,visc_col,x_center,y_center,ymax)
    # print("slab viscosity = %e Pas at (x,y) = (%.0f, %.0f) km" % (slab_viscosity,slab_viscosity_x,slab_viscosity_y))
    ...
    dips_unsmoothed = get_dip_slab_midplane(llith_points)
    dips = savgol_filter(dips_unsmoothed[:,0],601,3)
    K, dK, K_unsmoothed, dK_unsmoothed = get_curvature_slab_midplane(llith_points,dips)
    ...
    # calculate bending stress profile
    bending_strainrate = (0.25 * slabnorm_thick) * (vc/(100.*365*24*60*60)) * dK  # s^-1
    bending_stress     = 2. * slab_viscosity * bending_strainrate                 # Pa
    # get bending stress at mid-mant depth
    bending_stress_midmant, xloc_stress, sloc_stress = get_stress_at_certain_depth(bending_stress,llith_points,analysis_depth)
    ...
    # calculate slab-normal normal stress gradient (to see whether continuum approximation OK)
    slab_norm_fullstress_cut            = slab_norm_fullstress[inds]
    slab_norm_fullstress_shall_cut      = slab_norm_fullstress_shall[inds_shall]
    slab_norm_fullstress_deep_cut       = slab_norm_fullstress_deep[inds_deep]
    slab_norm_fullstress_gradient,       gradient_diff       = get_stress_gradient(slab_norm_fullstress_cut,profile_cut)
    slab_norm_fullstress_shall_gradient, gradient_shall_diff = get_stress_gradient(slab_norm_fullstress_shall_cut,profile_shall_cut)
    slab_norm_fullstress_deep_gradient,  gradient_deep_diff  = get_stress_gradient(slab_norm_fullstress_deep_cut, profile_deep_cut)
    gradient_diff_avg = 0.25*(gradient_shall_diff) + 0.25*(gradient_deep_diff) + 0.5*(gradient_diff)
    # print("gradient difference in slab-normal stess = %.2f MPa/km" % (gradient_diff_avg))

    # calculate curvature term for dp
    curvature_term = 0.5*(slabnorm_thick*K_midmant) * (Pleft+Pright)

    saved_stresses[ind,:] = time_dim, DP_mod_shall, DP_mod_deep, DP_mod, DP_anal, dip_midmant, slab_stress_term, slab_stress_term_b, \
                            slab_stress_term_c, slabnorm_thick, Snorm_contrib, gradient_diff_avg, K_midmant, dK_midmant, vc, slab_viscosity, \
                            bending_stress_midmant, bending_stress_midmant/DP_anal, Pleft, Pright, curvature_term
    ind = ind + 1
    ...
    ax1.annotate(''.join([str("%.1f" % (time_dim)),' Myr']), xy=(0.01,0.6), xycoords='axes fraction',verticalalignment='center',horizontalalignment='left',fontsize=10,color='k')       
    ax1.annotate(''.join(['DP: mod = ',str("%.1f" % (DP_mod/1.e6)),', anal = ',str("%.1f" % (DP_anal/1.e6)),', missing =',str("%.1f" % (DP_missing/1.e6)),' MPa']), xy=(0.01,0.12), xycoords='axes fraction',verticalalignment='center',horizontalalignment='left',fontsize=8,color='k')        
    ax1.annotate(''.join(['vc = ',str("%.1f" % (vc)),' cm/yr']), xy=(0.98,0.12), xycoords='axes fraction',verticalalignment='center',horizontalalignment='right',fontsize=8,color='k')
    ax1.plot(llith_points[:,0], llith_points[:,1], linewidth=0.5, color='black',zorder=6)
    ax1.plot(profile_shall[:,0],profile_shall[:,1], linewidth=0.5, color='green',zorder=6)
    ax1.plot(profile_deep[:,0], profile_deep[:,1],  linewidth=0.5, color='blue',zorder=6)
    ax1.plot(profile[:,0],      profile[:,1],       linewidth=0.5, color='red',zorder=6)
    ...
    # plot mid-plane dip
    ax2=fig.add_subplot(gs[1,0])
    ax2.set_ylabel("dip [degrees]",size=6.5)
    # ax2.set_xlim(50,300); 
    # ax2.set_ylim(45,90); 
    ax2.tick_params(axis='x', labelsize=6)
    ax2.tick_params(axis='y', labelsize=6)
    ax2.plot(llith_points[:,2],dips, color='black', linestyle='-', linewidth=1.5, zorder=2)
    ax2.scatter(sloc_dip,dip_midmant, color='red', s=15, lw=0, zorder=3)
    ax2.plot(llith_points[:,2],dips_unsmoothed, color='gray', linestyle='-', linewidth=1, zorder=1)
    ax2.legend(fontsize=6,loc='right',facecolor='white',fancybox=True, framealpha=1)

    # plot mid-plane K
    ax3=fig.add_subplot(gs[2,0])
    ax3.set_ylabel("K [rads/m]",size=6.5)
    # ax3.set_xlim(50,300); 
    # ax3.set_ylim(-0.5,6); 
    ax3.tick_params(axis='x', labelsize=6)
    ax3.tick_params(axis='y', labelsize=6)
    ax3.plot(llith_points[:,2],K,color='black', linestyle='-', linewidth=1.5, zorder=2)
    ax3.scatter(sloc_K,K_midmant, color='red', s=15, lw=0, zorder=3)
    ax3.plot(llith_points[:,2],K_unsmoothed,color='gray', linestyle='-', linewidth=1, zorder=1)
    # ax3.axhline(y=0, color='gray',linestyle='--',linewidth=1, zorder=1)

    # plot mid-plane K
    ax4=fig.add_subplot(gs[3,0])
    ax4.set_ylabel("dK/ds [rads/m^2]",size=6.5)
    ax4.set_xlabel("s [km]",size=6.5)
    # ax4.set_xlim(50,300); 
    ax4.tick_params(axis='x', labelsize=6)
    ax4.tick_params(axis='y', labelsize=6)
    ax4.plot(llith_points[:,2],dK,color='black', linestyle='-', linewidth=1.5,zorder=2)  
    ax4.plot(llith_points[:,2],dK_unsmoothed,color='gray', linestyle='-', linewidth=1,zorder=1)  
    ax4.scatter(sloc_K,dK_midmant, color='red', s=15, lw=0, zorder=3)
    # ax4.annotate(''.join(['slab stress term = ',str("%.1f" % (slab_stress_term/1.e6)),' MPa']), xy=(0.01,0.12), xycoords='axes fraction',verticalalignment='center',horizontalalignment='left',fontsize=8,color='k')        
    # ax4.axhline(y=0, color='gray',linestyle='--',linewidth=1, zorder=1)

    # # plot vp
    # ax5=fig.add_subplot(gs[4,0])
    # ax5.set_ylabel("vp [cm/yr]",size=6.5)
    # ax5.set_xlabel("x [km]",size=6.5)
    # ax5.tick_params(axis='x', labelsize=6)
    # ax5.tick_params(axis='y', labelsize=6)
    # ax5.plot(surf_prof[:,x_col]/1.e3,surf_prof[:,vx_col]*100,color='black', linestyle='-', linewidth=1.5,zorder=2)
    # ax5.plot([(trench_loc/1.e3)-1000, trench_loc/1.e3], [vsp, vsp], color='red', linewidth=2, zorder=1)
    # ax5.plot([(trench_loc/1.e3), (trench_loc/1.e3)+1000], [vop, vop], color='red', linewidth=2, zorder=1)
    # # print("saving fields figure to %s..." % plotname)
    # plt.savefig(curvature_plotname, bbox_inches='tight', format='png', dpi=500)
    # plt.clf()

    # plot bending stress
    ax5=fig.add_subplot(gs[4,0])
    ax5.set_ylabel("bending/buoy",size=6.5)
    # ax5.set_ylim(-1,5); 
    ax5.set_xlabel("x [km]",size=6.5)
    ax5.tick_params(axis='x', labelsize=6)
    ax5.tick_params(axis='y', labelsize=6)
    ax5.plot(llith_points[:,2],(bending_stress/DP_anal),color='black', linestyle='-', linewidth=1.5,zorder=2)
    ax5.scatter(sloc_stress,bending_stress_midmant/DP_anal, color='red', s=15, lw=0, zorder=3)
    # print("saving fields figure to %s..." % plotname)
    plt.savefig(curvature_plotname, bbox_inches='tight', format='png', dpi=500)
    plt.clf()
