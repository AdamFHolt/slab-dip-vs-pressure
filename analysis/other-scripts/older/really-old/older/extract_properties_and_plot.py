...
first_time=8
saved_stresses = np.zeros(((max_time-first_time),13)) # time [Myr], DP_mod_shall, DP_mod_deep, DP_mod, DP_anal, dip, stress term, stress term B, stress term C,slabnorm thick, snorm contrib, dgradient normal stress, slab curvature
ind = 0 
...
for time in range(first_time,max_time,1):
    ...
    # get slab curvature
    slab_curvature = (dip_midmant_deep-dip_midmant_shall)/(sloc_dip_deep-sloc_dip_shall)		   # deg/km
    print("curvature: ddip = %.2f degs; ds = %.2f km; K = %.3f deg/km" % (dip_midmant_deep-dip_midmant_shall,sloc_dip_deep-sloc_dip_shall,slab_curvature))
    ...
    saved_stresses[ind,:] = time_dim, DP_mod_shall, DP_mod_deep, DP_mod, DP_anal, dip_midmant, slab_stress_term, slab_stress_term_b, slab_stress_term_c, slabnorm_thick, Snorm_contrib, gradient_diff_avg, slab_curvature
    ind = ind + 1
