...
from functions import extract_horiz_prof, get_slablocation_from_horiz_prof_coremodels
from functions import get_nearslab_stresses_coremodels, get_dip_at_certain_depth
...
visc_col=27;      vx_col=0;           vy_col=1;
c_crust_col=23;   c_lith_col = 24;   
c_ucore_col = 25; c_lcore_col = 26
P_col = 30;     x_col=31;           y_col = 32;
...
first_time=8
saved_stresses = np.zeros(((max_time-first_time)+1,12)) # time [Myr], DP_mod_shall, DP_mod_deep, DP_mod, DP_anal, dip, stress term, stress term B, stress term C,slabnorm thick, snorm contrib, dgradient normal stress
ind = 0 
...
for time in range(first_time,max_time,1):
    ...
    lcore  = griddata((model_data[:,x_col], model_data[:,y_col]), model_data[:,c_lcore_col], (X_low2, Y_low2), method='linear')

    # get lithosphere contour, and then trim it to get slab mid-plane
    comp_contour_val = 0.5
    llith_cont = plt.contour(X_low2/1.e3, (ymax-Y_low2)/1.e3, lcore, levels=[comp_contour_val])
    ...
    x_left, y_left, x_center, y_center, x_right, y_right = \
        get_slablocation_from_horiz_prof_coremodels(midmant_prof,x_col,y_col,c_lith_col,c_lcore_col,ymax)
    ...
    x_left_shall, y_left_shall, x_center_shall, y_center_shall, x_right_shall, y_right_shall = \
        get_slablocation_from_horiz_prof_coremodels(midmant_prof_shall,x_col,y_col,c_lith_col,c_lcore_col,ymax)
    ...
    x_left_deep,  y_left_deep,  x_center_deep,  y_center_deep,  x_right_deep,  y_right_deep = \
        get_slablocation_from_horiz_prof_coremodels(midmant_prof_deep,x_col,y_col,c_lith_col,c_lcore_col,ymax)

    # get dip of mid-plane (and smooth)
    dips = get_dip_slab_midplane(llith_points)
    dips = savgol_filter(dips[:,0],601,3)
    ...
    Pleft, Pright, Sxxleft, Sxxright, Sxyleft, Sxyright, Pleft_x, Pright_x, Pleft_y, Pright_y, slab_x_left, slab_y_left, slab_x_right, slab_y_right = \
        get_nearslab_stresses_coremodels(y_center,slabnorm_thick_ul,slabnorm_thick_ll,dip_midmant,model_data,ds,ymax,x_col,y_col,P_col,c_lith_col,sxx_col,sxy_col,dz) # Pa
    ...
    Pleft_shall, Pright_shall, Sxxleft_shall, Sxxright_shall, Sxyleft_shall, Sxyright_shall, Pleft_x_shall, Pright_x_shall, Pleft_y_shall, Pright_y_shall, slab_x_left_shall, slab_y_left_shall, slab_x_right_shall, slab_y_right_shall = \
        get_nearslab_stresses_coremodels(y_center_shall,slabnorm_thick_ul,slabnorm_thick_ll,dip_midmant_shall,model_data,ds,ymax,x_col,y_col,P_col,c_lith_col,sxx_col,sxy_col,dz) # Pa
    Pleft_deep, Pright_deep, Sxxleft_deep, Sxxright_deep, Sxyleft_deep, Sxyright_deep, Pleft_x_deep, Pright_x_deep, Pleft_y_deep, Pright_y_deep, slab_x_left_deep, slab_y_left_deep, slab_x_right_deep, slab_y_right_deep = \
        get_nearslab_stresses_coremodels(y_center_deep, slabnorm_thick_ul,slabnorm_thick_ll, dip_midmant_deep, model_data,ds,ymax,x_col,y_col,P_col,c_lith_col,sxx_col,sxy_col,dz) # Pa 
    ...
    print(gradient_diff,gradient_shall_diff,gradient_deep_diff,gradient_diff_avg)
    print("gradient difference in slab-normal stess = %.2f MPa/km" % (gradient_diff_avg))

    saved_stresses[ind,:] = time_dim, DP_mod_shall, DP_mod_deep, DP_mod, DP_anal, dip_midmant, slab_stress_term, slab_stress_term_b, slab_stress_term_c, slabnorm_thick, Snorm_contrib, gradient_diff_avg
    ind = ind + 1
