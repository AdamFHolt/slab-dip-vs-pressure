...
plot_name_png = ''.join(['plots/DP-comparisons/compilations/',model_string,'-mods_z',str(analysis_depth/1.e3),'.shear-dz',str(analysis_depth_dz/1.e3),'.ds',str(ds/1.e3),'.prof-dz',str(dz/1.e3),'km.w-bending.png'])
plot_name_pdf = ''.join(['plots/DP-comparisons/compilations/',model_string,'-mods_z',str(analysis_depth/1.e3),'.shear-dz',str(analysis_depth_dz/1.e3),'.ds',str(ds/1.e3),'.prof-dz',str(dz/1.e3),'km.w-bending.pdf'])
...
bending_ration_thresh = 1.5
# free plates
for i in range(3,len(bothfree)):
    if np.abs(bothfree[i,17]) < bending_ration_thresh: 
...
for i in range(3,len(fixedSP)):
    if np.abs(fixedSP[i,17]) < bending_ration_thresh:     
...
for i in range(3,len(fixedOP)):
    if np.abs(fixedOP[i,17]) < bending_ration_thresh: 
...
ax.annotate('low bending', xy=(0.0125,0.93), xycoords='axes fraction',verticalalignment='center',horizontalalignment='left',fontsize=9,color='k') 
...
plt.ylabel("extracted stress  [MPa]",size=7.5)
# plt.legend(fontsize=5.5,loc='best',facecolor='white',fancybox=True, framealpha=1, frameon=False)
ax3.annotate('high bending', xy=(0.0125,0.93), xycoords='axes fraction',verticalalignment='center',horizontalalignment='left',fontsize=9,color='k') 
fixed_aspect_ratio(1)
