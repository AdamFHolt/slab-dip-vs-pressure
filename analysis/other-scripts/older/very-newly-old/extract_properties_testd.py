...
for time in range(first_time,max_time,1):
    ...
    slab_stress         = slab_force / (prof_length*1.e3) * slabnorm_thick
    prof_length_shall   = profile_shall_cut[len(profile_shall_cut)-1,2] - profile_shall_cut[0,2]
    slab_stress_shall   = slab_force_shall / (prof_length_shall*1.e3) * slabnorm_thick_shall
    prof_length_deep    = profile_deep_cut[len(profile_deep_cut)-1,2] - profile_deep_cut[0,2]
    slab_stress_deep    = slab_force_deep  / (prof_length_deep*1.e3) * slabnorm_thick_deep
    ...
    ddip = np.deg2rad(dip_midmant_deep) - np.deg2rad(dip_midmant_shall) # RADS
    R = 1.0/(K_midmant)                                                 # M/RADS
    ds_new = R * ddip                                                   # M
    print("ds new = %.1f km, ds old = %.1f km" % ((ds_shall+ds_deep)/1.e3,ds_new/1.e3))


    slab_stress_term = (slab_stress_deep - slab_stress_shall)/(ds_new)
    slab_stress_term_b = (slab_stress_deep - slab_stress)/(ds_deep)
    slab_stress_term_c = (slab_stress - slab_stress_shall)/(ds_shall)
    ...
    slabnorm_stress_tmpB            = trapz(slab_norm_devstress_cutB[:,0],       profile_cut[:,2]*1.e3) / (prof_length*1.e3)
    slabnorm_stressB                = slabnorm_stress_tmpB * slabnorm_thick
