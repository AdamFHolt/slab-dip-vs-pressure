...
plotname=''.join(['plots/evolution/simple/',str(model_name),'.',str(time),'.pressure.png'])
plotname_pdf=''.join(['plots/evolution/simple/',str(model_name),'.',str(time),'.pressure.pdf'])
...
xmin_plot=0; ymin_plot=0; grid_res=2.0e3
...
press  = griddata((model_data[:,x_col], model_data[:,y_col]), model_data[:,P_col], (X_low, Y_low), method='linear')
...
# get press ref
depth_press_ref = ymax - analysis_depth2      
dz = 30.e3                    
midmant_prof = model_data[model_data[:,y_col] < (depth_press_ref+dz)]
midmant_prof = midmant_prof[midmant_prof[:,y_col] > (depth_press_ref-dz)] 
midmant_prof = midmant_prof[midmant_prof[:,x_col].argsort()] # sort by x
press_ref    = midmant_prof[0,P_col]
...
press_plot = ax1.contourf(X_low/1.e3, (ymax-Y_low)/1.e3, (press-press_ref)/1.e6, cmap=cm.get_cmap('RdBu_r'),levels=np.linspace(-20,20,501),extend='both')
visc_contour = ax1.contour(X_low/1.e3, (ymax-Y_low)/1.e3, np.log10(visc), levels=[22], colors='black', linewidths=0.5)
# flow vectors
...
# color bar
cbar = plt.colorbar(press_plot, cax = fig.add_axes([0.92, 0.7, 0.015, 0.15]), ticks=[-20,0,20], ticklocation = 'right')
...
ax1.annotate(''.join([str("%.1f" % (time_dim)),' Myr']), xy=(0.98,0.115), xycoords='axes fraction',verticalalignment='center',horizontalalignment='right',fontsize=12,color='k') 
