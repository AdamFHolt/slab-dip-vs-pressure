#!/bin/python
# WITH-T VARIANT of plot_DPvsDP.color-points.no-ot.py, generated by hand from it.
# Identical to the original except that it reads text_files/TESTB_withT/
# (see make_TESTB_withT.py), in which column 17 carries the FULL curvature
# contribution K*H*(sigma_n_bar - P_ext) rather than its deviatoric part
# alone, and writes its figures with a .withT suffix.  Axis labels are
# unchanged because the proposed manuscript edit REDEFINES Qn rather than
# introducing a new symbol, so sigma_slab = K*Qn - dQs/ds still holds.
# The original script and its figures are untouched.
import numpy as np
import matplotlib
...
if depths == "normal":
    ...
    plot_name_png = ''.join(['plots/DP-comparisons/compilations/DP-vs-DP.dz',str(analysis_depth_dz/1.e3),'.ds',str(ds/1.e3),'.prof-dz',str(dz/1.e3),'km.tmin',str(tmin),'.200-to-400km.color-points.no-OT.withT.png'])
    plot_name_pdf = ''.join(['plots/DP-comparisons/compilations/DP-vs-DP.dz',str(analysis_depth_dz/1.e3),'.ds',str(ds/1.e3),'.prof-dz',str(dz/1.e3),'km.tmin',str(tmin),'.200-to-400km.color-points.no-OT.withT.pdf'])
elif depths == "other":
    analysis_depth1 = 250e3
    analysis_depth2 = 300e3
    analysis_depth3 = 350e3
    plot_name_png = ''.join(['plots/DP-comparisons/compilations/DP-vs-DP.dz',str(analysis_depth_dz/1.e3),'.ds',str(ds/1.e3),'.prof-dz',str(dz/1.e3),'km.tmin',str(tmin),'.250-to-350km.color-points.no-OT.withT.png'])
    plot_name_pdf = ''.join(['plots/DP-comparisons/compilations/DP-vs-DP.dz',str(analysis_depth_dz/1.e3),'.ds',str(ds/1.e3),'.prof-dz',str(dz/1.e3),'km.tmin',str(tmin),'.250-to-350km.color-points.no-OT.withT.pdf'])
else:
