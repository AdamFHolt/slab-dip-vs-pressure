...
if depths == "normal":
    ...
    plot_name_png = ''.join(['plots/DP-comparisons/compilations/DP-vs-DP.dz',str(analysis_depth_dz/1.e3),'.ds',str(ds/1.e3),'.prof-dz',str(dz/1.e3),'km.tmin',str(tmin),'.200-to-400kmB.no-OT.withT.png'])
    plot_name_pdf = ''.join(['plots/DP-comparisons/compilations/DP-vs-DP.dz',str(analysis_depth_dz/1.e3),'.ds',str(ds/1.e3),'.prof-dz',str(dz/1.e3),'km.tmin',str(tmin),'.200-to-400kmB.no-OT.withT.pdf'])
elif depths == "other":
    analysis_depth1 = 250e3
    analysis_depth2 = 300e3
    analysis_depth3 = 350e3
    plot_name_png = ''.join(['plots/DP-comparisons/compilations/DP-vs-DP.dz',str(analysis_depth_dz/1.e3),'.ds',str(ds/1.e3),'.prof-dz',str(dz/1.e3),'km.tmin',str(tmin),'.250-to-350kmB.no-OT.withT.png'])
    plot_name_pdf = ''.join(['plots/DP-comparisons/compilations/DP-vs-DP.dz',str(analysis_depth_dz/1.e3),'.ds',str(ds/1.e3),'.prof-dz',str(dz/1.e3),'km.tmin',str(tmin),'.250-to-350kmB.no-OT.withT.pdf'])
else:
...
# ---------------------------------------------------------------------------
# Fold in the curvature-pressure term T that Equation 2 drops:
#     T = (H/2)(dtheta/ds)(2*Pbar_n - P_subslab - P_wedge)
# computed by compute_curvature_pressure_term.py.  The top-row panels plot
# DP - dQs/ds + KQn (column 17 is the KQn term), so adding T into column 17
# turns them into the complete Equation 2 balance without touching any
# plotting function.  The bottom row (DP vs Bslab) uses columns 3 and 4 only
# and is therefore unchanged, as it should be: T does not enter the two-force
# balance.
first_time = 8
TDIR = 'text_files/curvature_pressure_term/'
T_COL = 15   # T_cut in the curvature-pressure output files


def add_T_inplace(model, model_name, depth):
    f = ''.join([TDIR, model_name, '.z', str(depth / 1.e3), '.txt'])
    if not os.path.exists(f):
        sys.exit('missing curvature-pressure file: %s\n'
                 'run: bash many_curvature-pressure-term.sh %s' % (f, depth))
    tdat = np.atleast_2d(np.loadtxt(f))
    for r in tdat:
        model[int(round(r[0])) - first_time, 17] += r[T_COL]
    return tdat.shape[0]


_GROUPS = [
    ('ref{d}',    (name_ref_bothfree,    name_ref_fixedSP,    name_ref_fixedOP)),
    ('ref{d}b',   (name_ref2_bothfree,   name_ref2_fixedSP,   name_ref2_fixedOP)),
    ('weak{d}',   (name_weak_bothfree,   name_weak_fixedSP,   name_weak_fixedOP)),
    ('strong{d}', (name_strong_bothfree, name_strong_fixedSP, name_strong_fixedOP)),
    ('new{d}',    (name_new_bothfree,    name_new_fixedSP,    name_new_fixedOP)),
]
_SUFFIX = ('_bothfree', '_fixedSP', '_fixedOP')

n_added = 0
for _d, _dep in ((1, analysis_depth1), (2, analysis_depth2), (3, analysis_depth3)):
    for _stem, _names in _GROUPS:
        for _suf, _nm in zip(_SUFFIX, _names):
            n_added += add_T_inplace(globals()[_stem.format(d=_d) + _suf], _nm, _dep)
print('folded curvature-pressure term T into %d model timesteps '
      '(3 depths x 15 models)' % n_added)
# ---------------------------------------------------------------------------
...
plt.ylabel(r'$\Delta P  +  \sigma_{slab} + T$   [MPa]')
